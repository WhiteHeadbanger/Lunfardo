...
class Chusma:
    ...
    def print_internals(self) -> None:
        ...
        for frame_name, context_for_frame in zip(self.get_stack(), self.get_context_stack()):
            ...
            frame_str = Chamuyo(f"│ Frame: {str(frame_name):<40}│")
            ...
            for sym, value in raw_symbols.items():
                self.res.register(self.matear_func.execute([Chamuyo(f'│   {sym:<12} → {str(value):<30}│')], self.context, self))
        ...
        morfar_value = self.res.register(morfar_func.execute([], self.context, self))
